...
typedef struct LNodeQ {//两个窗口的队列的元素，存储客户
	int ArriveTime;
	int DurTime;
...
void OpenForDay();//初始化
bool EventEmpty();//时间表为空，返回1；时间表不空，返回0
...
void CustomerArrive(LNodeE* q)
{
	...
	//	printf("加入新结点出了问题，以下为输出内容\n");
	//	printf("Window1.head->next == Window1.tail  %d\n", Window1.head->next == Window1.tail);
	//	printf("en.OccurTime < closetime  %d\n", en.OccurTime < closetime);
	//	printf("加入新结点出了问题，以上为输出内容\n");
	if (Window1.head->next == Window1.tail && en.OccurTime < closetime) {//第一个队伍只有一个顾客，需将这个顾客的办业务事件加入事件表

		en.NType = 1;
		EventInsert(en);
	}
	en.NType = 0; en.OccurTime = nexcus->ArriveTime + GetRandomT();
